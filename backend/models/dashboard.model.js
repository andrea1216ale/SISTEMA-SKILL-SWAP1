...
exports.getDashboard = async (userId) => {
  ...
  const [[userRows], [metricRows], [popularSkills], [recommendedUsers]] = await Promise.all([
    connection.query("SELECT id_usuario, CONCAT_WS(' ', nombres, apellido_paterno) AS nombre, correo FROM usuarios WHERE id_usuario = ? AND estado = 'ACTIVO' LIMIT 1", [userId]),
    connection.query(`
      SELECT
    ...
    `),
    connection.query(`
      SELECT u.id_usuario, CONCAT_WS(' ', u.nombres, u.apellido_paterno) AS nombre,
      ...
      WHERE u.id_usuario <> ? AND u.estado = 'ACTIVO' AND u.email_verificado = TRUE
